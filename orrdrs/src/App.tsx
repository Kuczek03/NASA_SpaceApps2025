...
const firebaseConfig = {
  apiKey: "AIzaSyDWo-nNmcMIPkmP_b54xRPaDt1AiYiawmM",
  authDomain: "orrdrs.firebaseapp.com",
  projectId: "orrdrs",
  storageBucket: "orrdrs.firebasestorage.app",
  messagingSenderId: "693427798575",
  appId: "1:693427798575:web:40e03d0c5dbebd6eedbcda",
  measurementId: "G-H1ZEFP0N1F"
};
...
const ORRDSSimulator = () => {
  ...
  const [opus, setOpus] = useState<OPU[]>([]);
  ...
  if (!newOpu.target) {
    ...
    if (availableDebris.length > 0) {
      // Fixed reduce with proper typing
      const closest = availableDebris.reduce<{ debris: Debris | null; dist: number }>((min, d) => {
  ...
  const newDebris: Debris[] = []; // Explicitly type the array
  for (let i = 0; i < 150; i++) {
    const randSize = Math.random();
    let size: 'small' | 'medium' | 'large' = 'small'; // Explicitly type the size variable
    if (randSize > 0.7 && randSize < 0.9) size = 'medium';
    else if (randSize >= 0.9) size = 'large';
    ...
    newDebris.push({
      ...
      distance: 100 + Math.random() * 180,
      speed: 0.001 + Math.random() * 0.003,
      size: size, // Now this matches the Debris interface
      value: size === 'small' ? 500000 : size === 'medium' ? 2000000 : 5000000,
      captured: false
    });
  }
